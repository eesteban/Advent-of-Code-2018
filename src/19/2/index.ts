...
async function parsePuzzle(input: ReadStream, registers: TRegisters) {
  ...
  return new Promise<IPuzzle>((res) => {
    lineReader.on('line', (line) => {
      ...
      .on('close', () => res({ ipIndex: ipIndex, ip: 0, instructions, registers }));
  });
}
...
function run(puzzle: IPuzzle) {
  while (true) {
    ...
    const { ip, registers } = puzzle;

    if (ip >= puzzle.instructions.length) {
      return registers[0];
    }
...
export async function start(src: string, initialRegisters: TRegisters) {
  const input = createReadStream(src);
  const start = +new Date();

  const puzzle = await parsePuzzle(input, initialRegisters);
  ...
  console.log("RESULT: the sum of factors of ", output);
